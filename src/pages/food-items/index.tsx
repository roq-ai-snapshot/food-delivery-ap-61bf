...
import { useRoqClient, useFoodItemFindManyWithCount } from 'lib/roq';
...
export function FoodItemListPage(props: FoodItemListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['menu', 'order_item.count'],
  ...
  const { data, error, isLoading, mutate } = useFoodItemFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.food_item.delete({ where: { id } });
      await mutate();
    } catch (error) {
