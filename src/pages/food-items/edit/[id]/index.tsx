...
import { useRoqClient, useFoodItemFindFirst } from 'lib/roq';
...
function FoodItemEditPage() {
  ...
  const { data, error, isLoading, mutate } = useFoodItemFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: FoodItemInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.food_item.update({
        data: values as RoqTypes.food_item,
        where: {
          id,
        },
      });
